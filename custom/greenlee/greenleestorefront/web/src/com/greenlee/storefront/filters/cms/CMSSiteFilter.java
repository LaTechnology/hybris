 ...
 *
 */
package com.greenlee.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
	...
	protected static final String SPLIT_PATH_PATTERN = "[/ ]";
	protected static final String CMS_FILTER_SKIP_CURRENCY_CHANGE_PROPERTY = "cmssitefilter.config.skip.currencychange";
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		if (cmsPageRequestContextData.getPreviewData() == null)
		{
			...
			if (processNormalRequest(httpRequest, httpResponse))
			{
				...
				final String currentLanguage = getCommerceCommonI18NService().getCurrentLanguage().getIsocode();
				final String currentCurrency = getCommerceCommonI18NService().getCurrentCurrency().getIsocode();
				final String newContextPath = "/" + currentLanguage + "/" + currentCurrency;

				LOG.info("The newContextPath is " + newContextPath + ": The request uri is" + httpRequest.getRequestURI());

				// Use the encoding attributes from session eg: /en/USD
				//If the patterns do not match, then detect a currency/locale change.
				if (!httpRequest.getRequestURI().startsWith(newContextPath) && !httpRequest.getRequestURI().endsWith("server-error"))
				{
				...
				if (!currencyChanged)
				{
					LOG.info("Encoding attributes not changed");
					// 1. If currency is not changed and if url is not changed then pass on to next filter.
					// 2. If currency is not changed but url has changed(this can happen if short urls/cms links are used)
					//    then append the query string & path variables to the url before redirecting.
					if (!httpRequest.getRequestURI().contains(newContextPath))
					{
						restoreQueryStringPathVariables(httpRequest, httpResponse, filterChain, newContextPath);
					}
					else
					{
				...
				{
					LOG.info("Encoding attributes changed");
					// 2. If currency is changed/url has changed(this can happen if short urls/cms links are used)
					//    then append the query string & path variables to the url before redirecting.
					restoreQueryStringPathVariables(httpRequest, httpResponse, filterChain, newContextPath);
				}
			}
	 ...
	 *
	 */
	private void restoreQueryStringPathVariables(final HttpServletRequest httpRequest, final HttpServletResponse httpResponse,
			final FilterChain filterChain, String newContextPath) throws ServletException, IOException
	{
		newContextPath += trimUrlEncodingAttributes(httpRequest.getRequestURI());

		if (StringUtils.isNotBlank(httpRequest.getQueryString()))
		{
			newContextPath = newContextPath + "?" + httpRequest.getQueryString();
		}
		if (StringUtils.equals(newContextPath, httpRequest.getRequestURI()))
		{
			filterChain.doFilter(httpRequest, httpResponse);
		}
		else
		{
			LOG.info("URL to be redirected to:" + newContextPath);
			httpResponse.sendRedirect(newContextPath);
		}
	}

	private String trimUrlEncodingAttributes(final String uri)
	{
		final BaseStoreModel baseStore = baseStoreService.getCurrentBaseStore();
		String trimmedUri = uri;
		for (final LanguageModel language : baseStore.getLanguages())
		{
			for (final CurrencyModel currency : baseStore.getCurrencies())
			{
				final StringBuilder urlBuilder = new StringBuilder();
				urlBuilder.append("/").append(language.getIsocode()).append("/").append(currency.getIsocode());
				trimmedUri = StringUtils.replace(trimmedUri, urlBuilder.toString(), "");
			}
		}
		return trimmedUri;
	}
			...
			throws IOException
	{
		...
		else if (!StringUtils.equals(SiteChannel.B2B.getCode(), cmsSiteModel.getChannel().getCode())) // Restrict to B2C channel
		{
	...
	protected boolean has2SkipURL(final HttpServletRequest request)
	{
		final List<String> skippedUrls = Arrays.asList(StringUtils.split(
				getConfigurationService().getConfiguration().getString(CMS_FILTER_SKIP_CURRENCY_CHANGE_PROPERTY, StringUtils.EMPTY),
				","));
	...
	/**
	 * @return the urlEncoderFacade
	 */
	public UrlEncoderFacade getUrlEncoderFacade()
	{
		return urlEncoderFacade;
	}

	/**
	 * @param urlEncoderFacade
	 *           the urlEncoderFacade to set
	 */
