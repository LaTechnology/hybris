...
/**
 * Filter that initializes the session for the greenleestorefront. This is a spring configured filter that is executed
...
public class StorefrontFilter extends OncePerRequestFilter
{
	...
	private static final Logger LOGGER = Logger.getLogger(StorefrontFilter.class);
			...
			final FilterChain filterChain) throws IOException, ServletException
	{
		...
		LOGGER.info("Strore front filter calling........" + isRequestSecure(request));
		...
		if (userFacade.isAnonymousUser() && redirectTo.contains(LOGOUT_URL))
		{
			request.setAttribute(ANONYMOUS_SESSION, new Boolean(true));
		}
			...
			final HttpServletResponse httpServletResponse)
	{
		...
		LOGGER.info("isRequestSecure >> " + isRequestSecure(httpServletRequest));
		LOGGER.info("CookieGenerator >> CookieHttpOnly >> " + getCookieGenerator().isCookieHttpOnly());
		LOGGER.info("CookieGenerator >> isCookieSecure >> " + getCookieGenerator().isCookieSecure());
