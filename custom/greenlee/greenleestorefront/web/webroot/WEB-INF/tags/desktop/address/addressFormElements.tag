...
<%@ taglib prefix="ycommerce" uri="http://hybris.com/tld/ycommercetags"%>
		...
		<formElement:formSelectBox idKey="address.title"
			...
			skipBlank="true" skipBlankMessageKey="Select"
			items="${titles}" selectedValue="${addressForm.titleCode}"
			selectCSSClass="custom-select" divCSSClass="title-select isBorder" />
			</div>
		</div>
		<div class="row">
			<div class="col-md-4">
			...
			<div class="col-md-4">
				<div id="countrySelector">
					<formElement:formSelectBox idKey="address.country" labelKey="address.country" path="countryIso" mandatory="true"
								skipBlank="false" skipBlankMessageKey="" items="${countries}" itemValue="${useShortRegionIso ? 'isocodeShort' : 'isocode'}"
								selectedValue="${addressForm.countryIso}" selectCSSClass="countrySelector form-control custom-select" divCSSClass="usertype-selector isBorder" />
					<spring:theme  code="address.country.disclaimer"/>
				</div>
			</div>
			<div class="col-md-4">
				<formElement:formSelectBox idKey="address.region"
							labelKey="address.state" path="regionIso" mandatory="true" skipBlank="false" skipBlankMessageKey="form.select.empty"
							items="${regions}" itemValue="${useShortRegionIso ? 'isocodeShort' : 'isocode'}" selectedValue="${addressForm.regionIso}"
							selectCSSClass="countrySelector form-control custom-select" divCSSClass="usertype-selector isBorder" />
				<spring:theme  code="address.country.disclaimer"/>
			</div>					
		</div>
		<div class="row">
