	...
	<div class="facet js-facet">
		...
		<div class="facet-values js-facet-values js-facet-form">
			...
			<ul class="facet-list js-facet-list <c:if test="${not empty facetData.topValues}">facet-list-hidden js-facet-list-hidden</c:if>">
				...
				<c:forEach items="${facetData.values}" var="facetValue" varStatus="theCount">
					<li>
						<c:if test="${facetData.multiSelect}">
							...
							<form action="#" method="get">
								...
								<div class="checkbox">
									<input id="checkbox${theCount.count+1}" type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''}  class="facet-checkbox js-facet-checkbox" />
								<label for="checkbox${theCount.count+1}">
									<!-- <span class="facet-label">
										<span class="facet-mark"></span>
										<span class="facet-text"> -->
											${facetValue.name}&nbsp;
						...
						<c:if test="${not facetData.multiSelect}">
							<c:url value="${facetValue.query.url}" var="facetValueQueryUrl"/>
							<span class="facet-text">
								<a href="${fn:escapeXml(facetValueQueryUrl)}">${facetValue.name}</a>
								<ycommerce:testId code="facetNav_count">
									<span class="facet-value-count"><spring:theme code="search.nav.facetValueCount" arguments="${facetValue.count}"/></span>
								</ycommerce:testId>
							</span>
