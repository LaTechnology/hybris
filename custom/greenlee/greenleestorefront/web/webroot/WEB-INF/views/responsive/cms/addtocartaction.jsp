...
<button class="compare-prod" onclick="addProducts(${product.code})">Compare</button><span id="com-message"></span> 
...
<form:form method="post" id="addToCartForm" class="add_to_cart_form span-5" action="${addToCartUrl}">
	...
	<c:if test="${empty showAddToCart ? true : showAddToCart}">
		...
		<c:choose>
		<c:when test="${product.showAddToCart}">
		<c:choose>
			...
			<c:otherwise>
				<button id="addToCartButton" type="${buttonType}" class="btn btn-primary btn-block js-add-to-cart">
		...
		<c:otherwise>
			<a href="#" title="ContactUs"><spring:theme code="home.contactus"/></a>
		</c:otherwise>
		</c:choose>		
	</c:if>
	<c:if test="${product.requestToDemo}">
		<a href="#" title="RequestToDemo"><spring:theme code="request.to.demo"/></a>
	</c:if>
</form:form>
